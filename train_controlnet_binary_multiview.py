...
# 计算多视角损失函数
def calculate_multiview_losses(generated_images, real_images, vgg_model):
    total_pixel_loss, total_style_loss, total_perceptual_loss, total_angle_consistency_loss = 0, 0, 0, 0
    ...
    for i in range(num_views):
        ...
        gen_features = extract_features(vgg_model, gen_img)
        real_features = extract_features(vgg_model, real_img)
        perceptual_loss = sum([F.mse_loss(g_feat, r_feat) for g_feat, r_feat in zip(gen_features, real_features)])
        total_perceptual_loss += perceptual_loss

        # 3. Style Loss
        style_loss = sum([F.mse_loss(gram_matrix(g_feat), gram_matrix(r_feat)) for g_feat, r_feat in zip(gen_features, real_features)])
        total_style_loss += style_loss

        # 4. Angle Consistency Loss (for consecutive views)
        if i < num_views - 1:
    ...
    total_pixel_loss /= num_views
    total_style_loss /= num_views
    total_perceptual_loss /= num_views
    total_angle_consistency_loss /= num_views - 1

    return total_pixel_loss, total_style_loss, total_perceptual_loss, total_angle_consistency_loss
...
def main(args):
    ...
    # )
    # RGB 数据的分组采样器
    rgb_sampler = GroupBatchSampler(rgb_train_dataset, group_size=group_size, batch_size=batch_size)
    rgb_train_dataloader = torch.utils.data.DataLoader(rgb_train_dataset, batch_sampler=rgb_sampler, collate_fn=collate_fn, num_workers=args.dataloader_num_workers)

    # Depth 数据的分组采样器
    depth_sampler = GroupBatchSampler(depth_train_dataset, group_size=group_size, batch_size=batch_size)
    depth_train_dataloader = torch.utils.data.DataLoader(depth_train_dataset, batch_sampler=depth_sampler, collate_fn=collate_fn, num_workers=args.dataloader_num_workers)
