...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value):
        # 强制所有输入和权重转换为 BFloat16
        query = query.to(torch.bfloat16)
        key = key.to(torch.bfloat16)
        value = value.to(torch.bfloat16)
        self.attention = self.attention.to(torch.bfloat16)  # 将注意力层的权重也转换为 BFloat16
...
vgg = models.vgg19(pretrained=True).features.to(accelerator.device).eval()
# 将模型参数转换为 bfloat16
...
# 定义一个辅助函数来提取特定层次的特征
def extract_features(x, layers=[4, 9, 18, 27]):
    features = []
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            ...
            # tracker.log({tracker_key: formatted_images})
                ...
                for input_image, generated_image in zip(input_images, generated_images):
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        # default=None,
        default=["A university building, realistic style, high-resolution, solid black background."],
        nargs="+",
        help=(
    ...
    parser.add_argument(
        ...
        # default=None,
        default=[[
            "/ssd/du_dataset/mvdfusion/my_dataset_processed_blender_whiteblock_512_60_angle_00/005/005_view_00.png",
            "/ssd/du_dataset/mvdfusion/my_dataset_processed_blender_whiteblock_512_60_angle_00/005/005_view_01.png",
            "/ssd/du_dataset/mvdfusion/my_dataset_processed_blender_whiteblock_512_60_angle_00/005/005_view_02.png",
            "/ssd/du_dataset/mvdfusion/my_dataset_processed_blender_whiteblock_512_60_angle_00/005/005_view_03.png"
        ]],
        nargs="+",
        help=(
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    grouped_dataset = MultiViewDataset(dataset["train"], group_size=4)
...
# 定义转换，将图像调整为 512x512，匹配预训练网络的标准输入
preprocess = transforms.Compose([
    transforms.Resize((224, 224))
])

def calculate_multiview_content_consistency_loss(generated_images, real_images, feature_extractor):
    """
    计算多角度生成图像之间的内容一致性损失，比较相邻角度生成图像与相邻角度真实图像的特征一致性。
    
    参数:
    - generated_images: 生成图像的张量列表，按角度顺序排列
    - real_images: 对应的真实图像张量列表
    - feature_extractor: 预训练的特征提取器（例如 VGG16）

    返回:
    - total_content_consistency_loss: 多角度内容一致性损失
    """
    num_views = len(generated_images)
    total_gen_real_consistency_loss, total_content_consistency_loss = 0, 0

    for i in range(num_views - 1):
        # 当前视角的生成图像和真实图像
        gen_img = preprocess(generated_images[i]).unsqueeze(0).to(torch.bfloat16)
        real_img = preprocess(real_images[i]).unsqueeze(0).to(torch.bfloat16)
        next_gen_img = preprocess(generated_images[i + 1]).unsqueeze(0).to(torch.bfloat16)
        next_real_img = preprocess(real_images[i + 1]).unsqueeze(0).to(torch.bfloat16)

        # print("gen_img shape:", gen_img.shape)  # 应该为 [1, 3, H, W]
        # print("real_img shape:", real_img.shape)
        # print("next_gen_img shape:", next_gen_img.shape)
        # print("next_real_img shape:", next_real_img.shape)

        # 提取特征
        gen_features = feature_extractor(gen_img)
        real_features = feature_extractor(real_img)
        next_gen_features = feature_extractor(next_gen_img)
        next_real_features = feature_extractor(next_real_img)

        # 比较相邻生成图像的特征和相邻真实图像的特征之间的一致性
        for gen_feat, real_feat, next_gen_feat, next_real_feat in zip(gen_features, real_features, next_gen_features, next_real_features):
            # 计算相邻生成图像的特征差和相邻真实图像的特征差之间的 MSE
            gen_real_feature_diff = F.mse_loss(gen_feat, real_feat)
            total_gen_real_consistency_loss += gen_real_feature_diff

            gen_feature_diff = F.mse_loss(gen_feat, next_gen_feat)
            real_feature_diff = F.mse_loss(real_feat, next_real_feat)
            content_consistency_loss = F.mse_loss(gen_feature_diff, real_feature_diff)
            total_content_consistency_loss += content_consistency_loss

    return total_gen_real_consistency_loss, total_content_consistency_loss
...
def calculate_angle_consistency_loss(generated_images, real_images):
    ...
    return total_angle_consistency_loss * 10
...
def calculate_multiview_losses(generated_images, real_images, vgg_model, true_generated_images, true_real_images, step = None):
    ...
    # 初始化特征提取器
    total_gen_real_consistency_loss, total_content_consistency_loss = calculate_multiview_content_consistency_loss(true_generated_images, true_real_images, extract_features)
    ...
    # 将返回的特征一致性损失进行平均处理
    total_gen_real_consistency_loss /= max(num_views, 1)  # 防止视角数为0
    total_content_consistency_loss /= max(num_views - 1, 1)  # 防止视角数小于2

    return pixel_loss, style_loss, perceptual_loss, color_loss, angle_consistency_loss, total_gen_real_consistency_loss, total_content_consistency_loss
...
def main(args):
    ...
    controlnet, optimizer, train_dataloader, lr_scheduler, unet = accelerator.prepare(
        controlnet, optimizer, train_dataloader, lr_scheduler, unet
    )
    ...
    # num_views = 60
    # 在训练循环中实现梯度累积
    accumulation_steps = 1  # 每个批次分为 6 个子组
    # accumulation_steps = 6  # 每个批次分为 6 个子组
    # accumulation_steps = 12  # 每个批次分为 6 个子组
    # 初始化交叉注意力层
    cross_attention_layer = CrossAttentionLayer(embed_dim=768, num_heads=8).to(device=accelerator.device)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(controlnet):
                ...
                # 执行交叉注意力，将不同视角的 encoder_hidden_states 共享到当前特征
                # 假设 encoder_hidden_states 的 shape 是 [batch_size * num_views, embed_dim]
                # 1. 展平 noisy_latents 的 spatial 维度，并转换为适配交叉注意力的形状
                # 备份 noisy_latents
                original_noisy_latents = noisy_latents.clone()  # 备份一份原始的 noisy_latents
                noisy_latents = noisy_latents.view(noisy_latents.shape[0], noisy_latents.shape[1], -1)  # [4, 4, 4096]
                noisy_latents = noisy_latents.permute(0, 2, 1)  # [4, 4096, 4]

                # 将通道数映射到与 encoder_hidden_states 一致的 768 维度
                linear_layer = nn.Linear(4, 768).to(noisy_latents.device, dtype=noisy_latents.dtype)
                noisy_latents = linear_layer(noisy_latents)  # [4, 4096, 768]
                encoder_hidden_states = encoder_hidden_states.repeat(1, 4096 // 77 + 1, 1)[:, :4096, :]
                cross_attention_output = cross_attention_layer(
                    query=noisy_latents,  # 将 noisy_latents 作为 query
                    key=encoder_hidden_states,
                    value=encoder_hidden_states
                )

                # 将交叉注意力输出添加到 noisy_latents 中
                noisy_latents = noisy_latents + cross_attention_output.view_as(noisy_latents)

                # 将通道数从 768 映射回 4
                linear_layer_back = nn.Linear(768, 4).to(noisy_latents.device, dtype=noisy_latents.dtype)
                noisy_latents = linear_layer_back(noisy_latents)  # [batch_size, 4096, 4]
                # 恢复形状为 [batch_size, 4, 64, 64]
                noisy_latents = noisy_latents.permute(0, 2, 1).view(bsz, 4, 64, 64)
                ...
                noisy_latents = original_noisy_latents
                ...
                # 确保 `denoised_latents` 和 `denoised_target_latents` 的类型与 `vae` 的权重类型一致
                denoised_latents = denoised_latents.to(vae.dtype)
                # denoised_target_latents = denoised_target_latents.to(vae.dtype)
                with torch.no_grad():
                    true_generated_images = vae.decode(denoised_latents).sample
                ...
                # 计算多视角损失
                pixel_loss, style_loss, perceptual_loss, color_loss, angle_consistency_loss, total_gen_real_consistency_loss, total_content_consistency_loss = calculate_multiview_losses(
                ...
                total_loss = base_loss + pixel_loss + angle_consistency_loss + color_loss
            ...
            logs = {
                ...
                "total_gen_real_consistency_loss": total_gen_real_consistency_loss.detach().item(),
                "total_content_consistency_loss": total_content_consistency_loss.detach().item(),
